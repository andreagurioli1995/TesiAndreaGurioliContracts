...
contract FeedBackedCall is owned, nameRegAware, loggable {
    ...
    // Underlier details il notional equivale alla quantità di quell'asset che voglio acquistare
    string public      _feedName;
    ...
    function exercise() returns (bool) {
        ...
        CashFlow(address(_buyerAcct),
                 address(_sellerAcct),
                 this.balance);
        _sellerAcct.transfer(this.balance);
        ...
        CashFlow(address(_sellerAcct),
                 address(_buyerAcct),
                 this.balance);
        _buyerAcct.transfer(this.balance);
    ...
    // ===== Utility functions ===== //
//controllo se ho raggiunto il tempo di maturity
...
//mi controlla se il prezzo del sottostante è inferiore o superiore allo strike price
...
//recupero il prezzo del sottostante
